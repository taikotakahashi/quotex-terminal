...
class AccountMixin:
    ...
    async def connect(self) -> tuple[bool, str]:
        ...
        # Browser TLS impersonation target for the curl_cffi WS transport.
        self.api.impersonate = getattr(self, "impersonate", "chrome")
        # Route the WS through a proxy too (not just the HTTP client), if set.
        self.api.proxy_url = getattr(self, "proxy_url", None)
        ...
        if not await self.check_connect():
            # Distinguish an app-level authorization rejection (expired/invalid
            # SSID) from a transport-level failure (Cloudflare 403 / network),
            # so callers can show the right guidance. The sticky flag survives
            # the reconnect that would otherwise reset auth_status.
            auth_rejected = (
                getattr(self.api, "auth_rejected", False)
                or self.api.state.auth_status == AuthStatus.FAILED
            )
            logger.error(
                "Websocket %s.",
                "authorization was rejected (session expired/invalid)"
                if auth_rejected else "failed to connect or was rejected",
            )
            if "token" in self.session_data:
                self.session_data["token"] = None
            if auth_rejected:
                return False, "authorization_rejected"
            # If the WS opened + engine.io handshook but Quotex neither authorized
            # nor sent an explicit reject (it just disconnected the namespace),
            # the account/session is being refused server-side — distinct from a
            # Cloudflare 403 (where the WS never opens at all).
            if getattr(self.api, "ws_ever_opened", False):
                return False, "session_refused"
