...
class QuotexAPI:
    ...
    async def _on_open(self) -> None:
        ...
        # The transport opened (so it is NOT a Cloudflare 403). Used by callers
        # to tell a transport failure apart from an app-level auth refusal.
        self.ws_ever_opened = True
    ...
    async def _on_message(self, msg: bytes | str) -> None:
        ...
        try:
            ...
            if "authorization/reject" in msg_str:
                ...
                # Sticky flag: survives the reconnect that resets auth_status, so
                # callers can reliably tell an expired-SSID rejection apart from a
                # transport failure. (LOCAL VENDOR PATCH — see VENDORED.md.)
